...
export const getProfileAnalytics = cartographerAddress =>
  axios
    ...
    .then(res => {
      ...
      const newPoints = cartographerHistory.filter(
        item => item.action === 'newPoint'
      ).length;
      const removedPoints = cartographerHistory.filter(
        item => item.action === 'removedPoint'
      ).length;
      const pointsChallenged = cartographerHistory.filter(
        item => item.action === 'challenge'
      ).length;
...
const resultChallenge = listingHash =>
  axios.get(`https://map-api-direct.foam.space/poi/details/${listingHash}`);
...
const getArcLayerData = cartographerJourneyObject => {
  ...
  for (i = 0; i < cartographerJourneyObject.length - 1; i += 1) {
    const pointCoordsSource = getPointCoords(
      cartographerJourneyObject[i].geohash
    );
    const pointCoordsDestination = getPointCoords(
      cartographerJourneyObject[i + 1].geohash
    );
    const sourceStatus = cartographerJourneyObject[i].state.status.type;
    const destinationStatus =
      cartographerJourneyObject[i + 1].state.status.type;
    const dateOfMarking = cartographerJourneyObject[i].state.createdAt;
    const stakedValue = hexToDecimal(
      cartographerJourneyObject[i + 1].stakedvalue
    );
...
export const fetchCartographerDetailsFromFOAMAPI = async cartographerAddress => {
  ...
  const detailOfAllChallengedPoints = allPointsOfCartographer.data.filter(
    item => item.action === 'challenge'
  );

  const hashOfAllChallengedPoints = detailOfAllChallengedPoints.map(
    item => item.listingHash
  );
  ...
  const newDataPromises = hashOfAllChallengedPoints
    .map(resultChallenge)
    .map(p => p.catch(() => { }));
  ...
  const allPointsInterectedByCartographer = filteredAddedPOIData.concat(
    challengedPoints
  );
  allPointsInterectedByCartographer.sort(
    (a, b) =>
